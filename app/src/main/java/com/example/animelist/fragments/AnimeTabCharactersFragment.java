...
public class AnimeTabCharactersFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.anime_tab_characters_layout, container, false);
    }
        ...
        if (activity != null) {
            ...
            sharedViewModel.getAnime().observe(getViewLifecycleOwner(), new Observer<Anime>() {
                        ...
                        public void onChanged(Anime anime) {
                            RecyclerView recyclerView = activity.findViewById(R.id.recyclerViewCharacters);
                            recyclerView.addItemDecoration(new SpacesItemDecoration(0, dpToPixels(activity, 10)));
                            CharacterAdaper adapter = new CharacterAdaper();
                            recyclerView.setAdapter(adapter);
                            adapter.setData(anime.getCharacters().getEdges());
                        }
                    });
        }
    }
